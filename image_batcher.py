    ...
    def preprocess_image(self, image_path):
        ...
        * EfficientDet: Resizes and pads the image to fit the input size.
        ...
        def resize_pad(image):
            ...
            :param image: The opencv image object (numpy)
            ...
            if not scale == 1:
                image = cv2.resize(image, (max(round(width * scale), self.width), max(round(height * scale), self.height)))
            n_h, n_w, _ = image.shape
            if not (n_h == height and n_w == width):
                image = cv2.copyMakeBorder(image, 0, n_h - height, 0, n_w - width, cv2.BORDER_CONSTANT, 0)
        ...
        image, scale = resize_pad(image)
        image = np.asarray(image, dtype=self.dtype)

        if self.normalization['enable']:
            cv2.subtract(image, self.mean, image)  # inplace
            cv2.multiply(image, self.stdinv, image)  # inplace

            # for i in range(image.shape[2]):
            #     image[:, :, i] -= self.normalization['mean'][i]
            #     image[:, :, i] /= self.normalization['std'][i]
